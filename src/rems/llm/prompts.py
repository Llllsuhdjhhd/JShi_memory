...
# =====================================================================
# 1. Event Former（形成状态划分）
# =====================================================================
# 只判断形成状态：events / residual / no_form。
# 字数门槛、封存原因、切开、搁置由程序处理。摘要与对象不在这次调用里。
...
残影是形成窗口里尚未形成独立事件的线。里面可以有多条线：明显没说完的，还在发展的，以及局部已经闭合、但后续仍可能属于同一段的。每条线有程序给出的标识。出现次数不参与判断。
...
1. 语义边界优先。在仍然内部一致的前提下，取尽可能长的一段作为一条事件。三句话可以是一条事件，只要它们是同一段经验。同一段还在继续时，整段留在残影，不要拆成多条短事件。
...
10. 结果来自某条已有线时，填写 continues 为那条线的标识，并把该线原有句子的序号写进同一条 indices。对 events，这表示该线在此形成为事件，或一条已形成的事件被后续句子扩入。对 residual，这表示仍是同一条正在形成的经验。没有来源线则为 null。
11. 不要因为一条线很长就自行按比例切开。整条仍是一段未完成的经验时，整段放进 residual。
12. 只返回句子序号，不返回句子原文。默认每个句子只属于一处。只有这句本身同时构成两段已经形成的经验的必要部分时，才可以出现在两条 events 里。
...
每条线的标识只可原样写入 continues。没有列出的标识不要发明。没有则本节为（无）。

## 句子
{range_hint}
{indexed_input}

句子序号从 1 开始。已形成的句子在前，其次是残影，再是待重判，最后是本轮新输入。区间含义见上一行。
...
你是匠石记忆后端的事件充实（Event Enrichment）组件。给定**已封存**经历原文，**单次输出**：
摘要层级（A）、主体（匠石/"我"）事件级情感（B）、每个对象一句事实（C，匠石视角）。**不抽取角色**——对象由输入映射表给定。

**视角统一**：本记忆是匠石的个人记忆，摘要与对象事实一律以匠石（"我"）第一人称视角叙述，不要混用第三人称"匠石"。
...
C. **objects（每个对象一句事实）**：视角一律是匠石（"我"）。每个对象只写一句 `fact`，写这一次发生了什么，并绑在本事件上。
   - 只写原文里已经出现的事实，例如「我和小明一起看了落日」。
   - 禁止写成长期特征，例如「性格急躁」「他是个温柔的人」。
   - 不要写三级快照，不要给对象写情感或等级。
   - 不要新增名单之外的对象，不要遗漏名单中的对象。
   - 若本段是切开或搁置后封存的片段，不要把事实写成这件事已经结束。
...
3. 生成 `objects`（每个对象一句事实 `fact`，匠石视角）。
