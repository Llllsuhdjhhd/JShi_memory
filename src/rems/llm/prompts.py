...
BOUNDARY_SYSTEM = """\
你是 REMS 事件剥离与边界检测组件。你的任务是从可能存在交叉、多线程或冗长的输入文本中，识别并剥离出：
（甲）真正逻辑完整、已闭环、可独立封存的事件；以及（乙）尚未闭环、应继续挂起的未完成片段。

核心任务：
1. **事件剥离与序号编码**：原文已按 `[序号] 文本` 格式进行了短句拆分。你必须通过返回 **句子序号列表** (`content_raw_indices` / `indices`) 来界定范围。
   - 允许不同事件共用同一个句子序号（内容覆盖）。
   - 禁止在 JSON 中返回原文 text。
   - **不要生成摘要、角色、情感等衍生字段**，这些由下游组件处理。
2. **防碎片化（优先挂起，勿滥封）**：
   - 同一叙事线索上的若干短句 / 对话回合，只要仍在同一件事上推进、尚未收束，应合并进**同一条** `new_unclosed`（或同一条 `completed`），不要「两句一件、再两句又一件」地切成大量 completed。
   - 只有明确构成**独立逻辑闭环**（因果与结果已齐、话题明确收束）时，才放入 `completed_events`。
   - 闲聊、吐槽进行中、承诺未兑现、问题未回答、动作未完成 → 默认挂 `new_unclosed`，留给后续续写。
3. **续写判定**：若当前片段是未完成事件库中某条的续写，填写 `continuation_of` 为对应未完成 ID；否则为 null。
4. **残影 / 当前输入边界**：用户消息会显式给出"既有残影序号区间"与"本轮新输入序号区间"。
   - 你必须把「残影 + 本轮输入」的全部分句序号划完：放进某条事件、续写并回某条旧线，或放进新的未完成线。
   - 续写时，旧线里被接上的句子序号也要写进该条 `content_raw_indices`。程序会用旧线原文合并，并去掉这些旧序号对应的句子，避免同一句存两份。
   - 没有任何序号领走的句子，程序会收成一条未完成线留下，不删除、不改写。不要指望程序丢掉你没提到的句子。
5. **未完成与残影统一**：与当前事件无关、或尚未闭环的逻辑片段，请统一填入 `new_unclosed`。系统后续将这些片段的拼接定义为"残影"。
6. **系统侧搁置封存（勿由你提前切碎）**：闲置过久时，程序会在边界检测之前按原文封存，原因记为 truncated，这不是自然闭环。因此：
   - 你不必因为「担心挂太久」而把未闭环内容提前塞进 `completed_events`；
   - **说话人更换 / 多人交替发言 ≠ 结束**，不要因此封存或拆条；
   - 单条字数变长时不要提前封存。过长的未完成按第 7 条在相对收束处切开；切不开就整条留在未完成里，由程序记为 truncated。
7. **超长未完成事件的强制分裂**：用户消息会给出 ``force_threshold`` 字符数阈值。
...
## 系统侧搁置封存（程序执行；你只需优先挂起未闭环内容）
闲置：字数 > {unclosed_idle_partial_ratio:.0%}×{unclosed_char_limit} 且闲置 ≥ {unclosed_idle_partial_days:g} 天，或闲置 ≥ {unclosed_idle_hard_days:g} 天 → 程序按原文封存，原因是 truncated。
字数变长本身不在边界检测前封存。过长请按 force_threshold 切开。
说话人更换不触发封存。

## 强制分裂阈值（仅针对超长未完成的 80/20）
...
单条未完成也可写 ``{{"indices": [12]}}``（同一条内多句请写进**同一**扁平列表，勿每句一条记录）。与当前事件无关但需保留的句子，也请放入 ``new_unclosed``。请把全部分句序号划完。程序会把没被领走的句子收成一条未完成线留下，不删除。优先把未闭环对话挂起，勿为「话题换了一下」就拆成多条 completed。"""
# ---------------------------------------------------------------------
...
请把全部分句序号划完。程序会把没被领走的句子收成一条未完成线留下，不删除。优先把未闭环对话挂起，勿为「话题换了一下」就拆成多条 completed。"""
# =====================================================================
